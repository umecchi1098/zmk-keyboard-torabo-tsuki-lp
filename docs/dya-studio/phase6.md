...
2026-08-15、標準の片側トラックボール構成を実機へ書き込み、次を確認した。

- DYA StudioでAML関連設定を変更できる
- Save／Discard／Resetと再起動後の復元が正常に動作する
- ポインター、Auto Mouse、Scroll、各マウスボタンが従来どおり動作する
- 確認後に既定値へ戻しても正常に動作する

任意の`double_ball`構成は自動検証済みだが、対象実機での左右ポインター確認は未実施である。標準配布構成は自動検証と必須実機確認をすべて通過したため、フェーズ6を完了とする。
