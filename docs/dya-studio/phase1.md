...
確認できた原因範囲は、旧customファームから引き継いだ左右間BLEの独自省電力処理である。この処理は最初のcustomファームから右Centralへ常時組み込まれ、無操作5秒・15秒・30秒の各段階で`bt_conn_le_param_update()`を呼び出していた。DYA向けZMK／Zephyr 4.1への移行時には入力コールバックの関数形式だけを更新し、省電力アルゴリズム自体は変更していなかった。

修正前は無操作後に左Peripheralが操作不能になり、独自処理だけを無効にした修正版では同じ症状が解消した。この実機結果から、切断の発生経路が独自の接続パラメータ更新処理であることは確認できた。

一方、`bt_conn_le_param_update()`の成功は接続パラメータ変更要求の受付を示すもので、実際の変更完了はBluetooth接続コールバックで別途確認する必要がある。旧処理は完了通知を確認せずに内部の電源モードを更新していたため、Zephyr 4.1のBluetoothスタックでは、実際の接続状態と内部状態が一致しない可能性がある。ただし、修正前ファームウェアではHCI切断理由を記録していないため、Peripheralによる拒否、監視タイムアウト、更新処理の競合のどれが最終的な切断理由だったかは未確定である。

### 安定版の運用方針

安定性を優先し、`CONFIG_TORABO_TSUKI_LP_SPLIT_POWER_MGMT`は既定で無効のまま運用する。この設定で無効になるのは旧custom由来の段階的な左右間BLEパラメータ変更だけであり、ZMK標準の省電力処理は引き続き有効である。

- 無操作30秒後: ZMK標準Idleへ移行し、BLE接続を維持したまま対象デバイスを休止する
- 無操作9,000,000ミリ秒（150分）後: ZMK標準Deep Sleepへ移行し、BLEを停止する
- Deep Sleep後: キー操作で起床し、左右間BLEを再接続する

独自処理を無効にすると、短時間の無操作中に左右間BLEの消費電力が従来より増える可能性がある。ただし、現行DYA2参照実装もZMK標準のIdle／Deep Sleepを使用しており、独自の段階的接続更新は使用していない。実測で電池寿命に問題が出るまでは、接続安定性を優先して現在の仕様を維持する。

電池寿命に問題が出た場合は、最初にDeep Sleepまでの時間を見直す。独自処理を再設計する場合は、接続パラメータ変更の完了確認、要求の直列化、失敗時の復旧、HCI切断理由の記録、実機での消費電流比較を必須とし、旧処理をそのまま再有効化しない。
...
2026-08-12に修正版を左右へ書き込み、電池駆動で無操作時間を置いた後も左Peripheralが操作可能で、左右間BLE接続が維持されることを確認した。この結果をもって、独自省電力処理を無効にした構成を安定版として採用する。

## 修正版の実機確認結果

- [x] 修正版の右Centralを書き込み、電池駆動で1分以上放置後も左キーが入力できることを確認する
- [x] Sleep復帰、USB、Bluetooth接続が通常運用へ進められる状態であることを確認する

以上の必須ゲートを満たしたため、フェーズ1を完了とする。
