...
- [x] 左Peripheralがクリーンビルド成功する（GitHub Actions #14）
- [x] 右Centralがクリーンビルド成功する（GitHub Actions #14）
- [x] Settings Resetがクリーンビルド成功する（GitHub Actions #14）
...
- [x] **ユーザー**: 左右へフェーズ1のUF2を書き込む
- [x] **ユーザー**: 接続中に全キーが入力できることを確認する
- [x] **ユーザー**: 修正版で左右分割通信を確認する（無操作後も左Peripheralが操作可能）
- [x] **ユーザー**: ポインター移動、Auto Mouse、Scrollレイヤー、左右クリック、MB4／MB5を確認する
- [x] **ユーザー**: Sleep復帰、USB、Bluetooth接続を確認する
- [x] **共同**: 2026-08-12、無操作後に左Peripheralが操作不能となる不具合を記録

### 実機回帰への対応

- [x] 独自の左右間BLE省電力処理が無操作5秒・15秒・30秒で接続条件を変更することを確認する
- [x] ZMK標準のIdle／Deep Sleep設定が有効であることを確認する
- [x] Zephyr 4.1では独自省電力処理を既定で無効にする
- [x] 修正版の左右・Settings Resetをローカルでクリーンビルドする
- [x] 修正版のGitHub Actionsが成功する（Actions #15）
- [x] **ユーザー**: 修正版の右Centralを書き込み、電池駆動で1分以上放置後も左キーが入力できることを確認する
- [x] **共同**: 独自BLE省電力処理を無効にした構成を安定版として運用する方針を確定する
...
- [x] GitHub Actionsで4成果物がクリーンビルド成功する（Actions run `31600347535`）
- [x] ローカルとCIの4成果物がSHA-256で完全一致する

### ユーザー実機確認 — 必須ゲート

- [x] **ユーザー**: USBでDYA Studioへ接続する
- [x] **ユーザー**: ロック中にKeymap変更が拒否されることを確認する
- [x] **ユーザー**: AdjustレイヤーからUnlockする
- [x] **ユーザー**: Keymapを変更し、保存できることを確認する
- [x] **ユーザー**: Mレイアウトとトラックボールの表示・動作を確認する
- [x] **共同**: 通常運用へ進められることを確認し、結果を記録する

### コミット

- [x] `ZMK Studioの安全なキーマップ編集に対応`
- [x] `ZMK Studio実機確認結果を反映`
...
- [x] 設定リセット用成果物がビルドできる
- [x] GitHub Actionsで4成果物がクリーンビルド成功する（Actions run `31603615015`）
- [x] ローカルとCIの4成果物がSHA-256で完全一致する
...
- [x] RAM／Flash使用量が許容範囲内である
- [x] GitHub Actionsで4成果物がクリーンビルド成功する（Actions run `31609107142`）
- [x] ローカルとCIの4成果物がSHA-256で完全一致する

### ユーザー実機確認 — まとめて1回

- [x] **ユーザー**: Runtime Macroを作成・実行する
- [x] **ユーザー**: Macroを保存し、再起動後に復元されることを確認する
- [x] **ユーザー**: Runtime Comboを作成・実行する
- [x] **ユーザー**: 既存BT Clearコンボが従来どおり動作することを確認する
- [x] **ユーザー**: Input Streamで押下位置とレイヤー変更が正しく表示されることを確認する
- [x] **共同**: テスト用Macro／Comboを削除し、結果を記録する

### コミット

- [x] `DYA Studioのマクロとコンボ編集に対応`
- [x] `マクロとコンボの実機確認結果を反映`
...
- [x] GitHub Actionsで6成果物がクリーンビルド成功する（Actions run `31857296764`）
- [x] ローカルとCIの6成果物がSHA-256で完全一致する

### ユーザー実機確認 — 必須ゲート

- [x] **ユーザー**: 標準版で通常のポインター移動を確認する
- [x] **ユーザー**: 標準版でAuto Mouseを確認する
- [x] **ユーザー**: Scrollレイヤーと方向・速度を確認する
- [x] **ユーザー**: DYA StudioからAML関連設定を変更できることを確認する
- [x] **ユーザー**: Save／Discard／Resetと再起動後の復元を確認する
- [x] **ユーザー**: 左右クリック、Middle、MB4、MB5を含む従来動作を確認する
- [x] **共同**: 既定値へ戻して結果を記録する（2026-08-15）

任意の`double_ball`構成は自動検証済みだが、対象実機を使用していないため左右ポインターの実機確認は未実施とする。標準配布構成の完了条件には含めない。
...
- [x] `DYA Studioの接続管理とDefault Layerに対応`
- [ ] Settings実装時に `DYA Studioの本体設定に対応`
...
|---|---|---|---|---|---|
| 2026-08-12 | フェーズ1 | GitHub Actions | `8250972`／Actions #14 | 成功 | 左・右・Settings Resetと成果物統合が成功 |
| 2026-08-12 | フェーズ1 | ユーザー実機 | `8250972`／Actions #14 `firmware` | 失敗 | 無操作後に左Peripheralが操作不能となる回帰を確認 |
| 2026-08-12 | フェーズ1 | ローカルDocker | 右Central SHA-256 `44be737e...` | 成功 | 独自BLE省電力処理が無効であることを生成Kconfigとビルドログで確認 |
| 2026-08-12 | フェーズ1 | GitHub Actions | `9a13493`／Actions #15 | 成功 | 修正版3成果物と成果物統合が成功。右Central SHA-256 `0c2fdbc7...` |
| 2026-08-12 | フェーズ1 | ユーザー実機 | `9a13493`／Actions #15 `firmware` | 成功 | 電池駆動で無操作後も左Peripheralが操作可能。独自BLE省電力処理を無効にした構成を安定版として採用 |
| 2026-08-12 | フェーズ1 | ユーザー実機 | 接続確認 | 成功 | Sleep復帰、USB、Bluetooth接続は通常運用へ進められる状態と判断 |
